...
from sgan.losses import gan_g_loss, gan_d_loss, l2_loss, gradient_penalty
...
def main(args, WGAN=False, WGAN_GP=False):
    ...
    if WGAN:
        optimizer_g = optim.RMSprop(generator.parameters(), lr=args.g_learning_rate)
        optimizer_d = optim.RMSprop(discriminator.parameters(), lr=args.d_learning_rate)
        args.clipping_threshold_d = 0.001
    else:
    ...
    while t < args.num_iterations:
        ...
        for batch in train_loader:
            ...
            if t > 0 and t % args.checkpoint_every == 0:
                ...
                plot_training_losses(checkpoint['D_losses']['D_data_loss'], checkpoint['G_losses']['G_discriminator_loss'], checkpoint['metrics_val']['ade'], checkpoint['metrics_val']['fde'], iteration=t, save_dir='../results/')
                logger.info('Done.')
...
def discriminator_step(
    ...
    if WGAN_GP:
        data_loss = data_loss + gradient_penalty(args.batch_size, discriminator, traj_real, traj_fake,traj_real_rel,traj_fake_rel)
    ...
    loss.backward()
    if args.clipping_threshold_d > 0 and not WGAN:
        nn.utils.clip_grad_norm_(discriminator.parameters(),
                                 args.clipping_threshold_d)
    if WGAN:
        for p in discriminator.parameters():
            p.data.clamp_(-args.clipping_threshold_d, args.clipping_threshold_d)
    optimizer_d.step()
...
def plot_training_losses(d_losses, g_losses, g_ades, g_fdes, iteration, save_dir='../results/'):
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 10), num=iteration)
    ax1.scatter(np.linspace(0, iteration, len(d_losses)) * iteration, np.asarray(d_losses))
    ax1.grid(True)
    ax1.set_xlabel("Discriminator loss")
    ax2.scatter(np.linspace(0, iteration, len(g_losses)) * iteration, np.asarray(g_losses))
    ax2.grid(True)
    ax2.set_xlabel("Generator loss")
    ax3.scatter(np.linspace(0, iteration, len(g_ades)) * iteration, np.asarray(g_ades))
    ax3.grid(True)
    ax3.set_xlabel("Generator ade")
    ax4.scatter(np.linspace(0, iteration, len(g_fdes)) * iteration, np.asarray(g_fdes))
    ax4.grid(True)
    ax4.set_xlabel("Generator fde")
