...
def generate_boundary_points(data_folder, annotated_image_name, annotated_image_name_out="world_points_boundary.npy", test=False):
    for root, dirs, files in os.walk(data_folder):

        if root != data_folder:
            break;

        for scene_folder in sorted(dirs):
            if scene_folder in ('hyang_3', 'hyang_4', 'hyang_9', 'nexus_0'):
                continue
            ...
            if test == True:
                ...
                plt.show()
            print("\n***** saving world_boundary_points:\n", root + scene_folder + "/" + annotated_image_name_out)
            np.save(root + scene_folder + "/" + annotated_image_name_out, world_boundary_points)


def main():
    data_folder = '/home/q467565/Desktop/FLORA/code/social_gan/datasets/safegan_dataset/SDD/'
